...
namespace Voluntr.Crosscutting.Infrastructure.Repositories
{
    public abstract class SqlRepository<TEntity>
        : ISqlRepository<TEntity> where TEntity : class, IEntity, new()
    {
        #region Properties
        ...
        {
            string orderProp = typeof(TEntity).GetProperty(pagination.OrderProperty).Name;
            ...
            var data = DbSet().Any()
                ? await Task.Run(() => DbSet()
                    .Order(orderExpression, pagination.OrderDesc)
                    .Skip(skip)
